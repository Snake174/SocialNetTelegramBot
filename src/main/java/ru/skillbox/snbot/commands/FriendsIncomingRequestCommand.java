...
@Slf4j
public class FriendsIncomingRequestCommand implements ICommand {
    ...
    public SendMessage getMessage(Update update) {
        try {
            CommonRs<?> response = client.getFriendsInReq(update.getCallbackQuery().getFrom().getId(), offset, perPage);

            if (response == null) {
                return Messages.siteUnavailable(update.getCallbackQuery().getMessage());
            }

            return Messages.friendsInReqMessage(update.getCallbackQuery().getMessage(), response);
