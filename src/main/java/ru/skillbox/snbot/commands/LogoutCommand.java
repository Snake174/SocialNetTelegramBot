...
public class LogoutCommand implements ICommand {
    ...
    public SendMessage getMessage(Update update) {
        TgApiRequest request = TgApiRequest.builder()
            ...
            .command(ECommands.KILLYOURSELF.getCommand())
            .build();
        ...
        try {
            ...
            if (response.getStatus().equals("fail")) {
                return Messages.failKillyourself(update.getCallbackQuery().getMessage(), response);
            }

            SessionStorage.setMode(update.getCallbackQuery().getFrom().getId(), EMode.LOGIN);

            return Messages.successKillyourself(update.getCallbackQuery().getMessage(), response);
