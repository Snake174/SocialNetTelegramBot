...
@Slf4j
public class FeedsCommand implements ICommand {
    ...
    public SendMessage getMessage(Update update) {
        try {
            CommonRs<?> response = client.getFeeds(update.getCallbackQuery().getFrom().getId(), offset, perPage);

            if (response == null) {
                return Messages.siteUnavailable(update.getCallbackQuery().getMessage());
            }

            return Messages.feedsMessage(update.getCallbackQuery().getMessage(), response);
