...
@Slf4j
public class DeclineFriendRequestCommand implements ICommand {
    ...
    public SendMessage getMessage(Update update) {
        try {
            CommonRs<?> response = client.declineFriendRequest(update.getCallbackQuery().getFrom().getId(), userId);

            if (response == null) {
                return Messages.siteUnavailable(update.getCallbackQuery().getMessage());
            }

            return Messages.declineFriendRequestMessage(update.getCallbackQuery().getMessage(), userName);
